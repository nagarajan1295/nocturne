...
# 192.168.44.1 gateway. Nocturne already brings up the controller at boot
# (udev: ttyS1 -> attach-bt -> btattach -P bcm; hci0 -> enable-bt) and runs
# bluetoothd (now with the `network` plugin -> org.bluez.Network1). All we do
# here is: wait for hci0, power on, pair+trust the Pi once (JustWorks via a
# NoInputNoOutput agent), connect the NAP, and give bnep0 a static address.
#
# This is the BT link. USB (usb0 = 192.168.7.2) stays up as the fallback path;
# start-chromium and birdmic_ct.py prefer BT (192.168.44.1) and fall back to
# USB (192.168.7.1) automatically, so the device works either way.
set -u
...
while :; do
  ...
  if ! paired; then
    log "pairing to ${PI_BT_MAC}"
    bluetoothctl --timeout 8 scan on >/dev/null 2>&1
    bluetoothctl pair  "$PI_BT_MAC" >/dev/null 2>&1
    bluetoothctl trust "$PI_BT_MAC" >/dev/null 2>&1
  fi

  if ! bnep_up; then
    log "connecting NAP"
    # Establish the ACL, then bring up the PANU role over D-Bus.
  ...
  if bnep_has_ip; then
    ...
    # Settle: poll the link, only act again if it drops.
